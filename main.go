...
type Logger struct {
	...
	Writer        LogWriter
	Color         bool
	// If this channel is loaded log 
	// entries are composed and sent to it
	LogChan chan Entry
}

// Entry is a log entry to be printed as 
...
// sanitizeLoglevel accepts a string and 
// returns a default if the input is not in 
// the Levels slice
...
func (w *LogWriter) Printf(format string, 
a ...interface{}) {
...
logPath, logFileName string) {
	...
	path := filepath.Join(logFileName, 
		logPath)
	...
	if FileExists(path) {
		...
		if err != nil {
			wr.Println(
				"error rotating log", err)
			return
		}
	}
	logFileHandle, err := os.
		OpenFile(path, 
...
// SetLevel enables or disables the various 
// print functions
func (l *Logger) SetLevel(
level string, color bool) *Logger {
	// *l = *Empty()
	...
	case level == Info || fallen:
		l.Info = printlnFunc(
			"INF", color, l.LogFileHandle, l.LogChan)
		l.Infof = printfFunc(
			"INF", color, 
			l.LogFileHandle, l.LogChan)
		l.Infoc = printcFunc(
...
func Composite(text, level string, color bool) string {
	...
	files := strings.Split(loc, "core/")
	var file string
	if len(files) > 1 {
		file = files[1]
	}
	sinceS := fmt.Sprint(
	time.Now().Sub(StartupTime) / 
		time.Second * 
		// this truncates to seconds
	...
	// print full time string on wide screen
	...
	if color {
		switch level {
		case "FTL":
			level = colorBold + colorRed + 
				level + colorOff
			since = colorRed + since + 
				colorOff
			file = colorItalic + colorBlue + 
				file
			line = line + colorOff
		case "ERR":
			level = colorBold + colorOrange + 
				level + colorOff
			since = colorOrange + since + 
				colorOff
			file = colorItalic + colorBlue + 
				file
			line = line + colorOff
		case "WRN":
			level = colorBold + colorYellow + 
				level + colorOff
			since = colorYellow + since + 
				colorOff
			file = colorItalic + colorBlue + 
				file
			line = line + colorOff
		case "INF":
			level = colorBold + colorGreen + 
				level + colorOff
			since = colorGreen + since + 
				colorOff
			file = colorItalic + colorBlue + 
				file
			line = line + colorOff
		case "DBG":
			level = colorBold + colorBlue + 
				level + colorOff
			since = colorBlue + since + 
				colorOff
			file = colorItalic + colorBlue + 
				file
			line = line + colorOff
		case "TRC":
			level = colorBold + colorViolet + 
				level + colorOff
			since = colorViolet + since + 
				colorOff
			file = colorItalic + colorBlue + 
				file
			line = line + colorOff
		case "STATUS":
			level = backgroundGrey + 
				colorBold + level + 
	...
	final := ""
	if levelLen+sinceLen+textLen+
	fileLen+lineLen > terminalWidth {
		lines := strings.Split(text, "\n")
		// log text is multiline
		line1len := terminalWidth - 
			levelLen - sinceLen - 
			fileLen - lineLen
		restLen := terminalWidth - 
			levelLen - sinceLen
		if len(lines) > 1 {
			final = fmt.Sprintf(
				"%s %s %s %s:%s", 
				level, since,
				strings.Repeat(" ",
					terminalWidth-levelLen-
					sinceLen-fileLen-lineLen),
				file, line)
...
// printfFunc prints a log entry with 
// formatting
...
// printcFunc prints from a closure 
// returning a string
func printcFunc(
	level string, 
	color bool, 
...
// FileExists reports whether the named 
...
// DirectionString is a helper function that 
// returns a string that represents the 
// direction of a connection (inbound or 
// outbound).
...
// PickNoun returns the singular or plural 
// form of a noun depending on the count n.
